...
window.GrvWidgetVars = function(siteGuid, placementId, userGuid, truncateArticleTitles, truncateArticleContent, isSidebar,
    ...
    sitePlacementId, googleAnalyticsWebPropertyId, googleAnalyticsOrganicClicks) {
...
window.grvInitWidget = function($, widgetVars) {
  ...
  function scoped$(selector) {
    return widgetVars.isIframe || !widgetVars.container ? $(selector) : $(selector, widgetVars.container);
  }
    ...
    if(!grvWatchingForHeightUpdated && postMessageSupported && widgetVars.frameUrl && parent !== window && widgetVars.widgetLoaderVersion >= 2) {
      ...
      $grvWindow.bind('message', function(event) {
        var data = event.originalEvent.data,
            ...
            lastWidth = $widget.data('lastWidth.grv'),
            lastHeight = $widget.data('lastHeight.grv'),
            curWidth = $widget.outerWidth(),
            curHeight = $widget.outerHeight();

        // Height updated
        if(data == 'heightUpdated' && widgetVars.frameUrl.indexOf(origin) == 0
        && (!lastWidth || lastWidth != curWidth || !lastHeight || lastHeight != curHeight)) {
          $widget.data('lastWidth.grv', curWidth).data('lastHeight.grv', curHeight);
          grvWidgetDimUpdated();
        }
      });
    }

    postParentMessage('setHeight:' + (widgetVars.useDynamicHeight ? $widget.outerHeight(true) : widgetVars.staticHeight));
  }
...
//        return;
      ...
      // Using Thumby
      if(widgetVars.useThumby || $link.data('campaignUsesThumby')) {
    ...
    var alignImage = function($link, $img, imageWidth, imageHeight) {
      ...
      // Image is more portraity
        ...
        // Check to see if we need to create BG image and blur it
        checkFullWidthRatio($img);
      }
    };
  ...
  function checkFullWidthRatio($img) {
  ...
  function bindArticleHandlers($grvparent) {
    var $forwardingLinks = $grvparent.find('[data-forward-href]');
    ...
    // Partner not capturing clicks; normal click behavior
      ...
      if (widgetVars.siteGuid == 'a1f9015c15922698596d7c5bdd1561c2') {
        $forwardingLinks.mousedown(rewriteHref);
      }
      // Non-ProBoards
      else {
  ...
  $(function() {
    ...
    bindArticleHandlers((widgetVars.container ? $(widgetVars.container) : null) || $('body'));
    ...
    if (widgetVars.isIframe) {
      ...
      widgetVars.doAolOmniture && aolOmniturePing();
    }
    // Non-iframe widget
    else {
      grvWidgetDimUpdated();
  ...
  function aolOmniturePing() {
    ...
    function omniPingWhenBnReady() {
      if(window.bN) {
        ...
        setTimeout(omniPingWhenBnReady, 50);
    }
    omniPingWhenBnReady();
...
(function() {
  // If this global isn't available, then that means this widget.js is running in conjunction with new server code
  // and widget_js.scaml will initialize itself; this only applies during deployment. The entire closure here can be
  // deleted later once the project is deployed fully to API and FAILOVER and stable for many days.
  if(!window.grvWidgetLoaderVersion)
    return;

  var widgetVars = new GrvWidgetVars(window.grvSiteGuid, window.grvPlacement, window.grvUserGuid,
          window.grvTruncateArticleTitles || true, window.grvTruncateArticleContent || false, window.grvIsSidebar || false,
          window.grvFrameUrl, window.grvWidgetLoaderVersion, window.grvUseDynamicHeight, window.grvStaticHeight,
          window.grvUseThumby, window.grvEnableImageTooltip, window.grvBrokenImgUrl, window.grvDoVerticalSpace,
          window.grvDoHorizontalSpace, window.grvIsIframeWidget, window.grvDoAolOmniture, window.grvClickThroughOnRightClick,
          window.grvShowMouseoverSlide, window.grvRateRecoBaseUrl, window.grvUseInnerScroll, window.grvImpressionHash,
          window.grvBeaconUrl, window.grvAolOmniModuleName || '', window.grvAolOmniBrand || '', window.grvAolCmsSrc || '',
          window.grvAolCobrand || '', window.grvAolOmniAccount, null);

  function initWidgetWhenJqueryReady() {
    if(window.$grv)
      grvInitWidget($grv, widgetVars);
    else
      setTimeout(initWidgetWhenJqueryReady, 10);
  }
  initWidgetWhenJqueryReady();
})();